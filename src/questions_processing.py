...
class QuestionsProcessor:
    def __init__(
        self,
        # 构造函数确定形参类型和默认值
        vector_db_dir: Union[str, Path] = './vector_dbs', # 向量数据库
        documents_dir: Union[str, Path] = './documents', # 原始文档
        # Optional表示它是可选形参
        questions_file_path: Optional[Union[str, Path]] = None, # 加上Optional就明确允许该形参可以是None
        new_challenge_pipeline: bool = False, # 用于切换是否使用新的处理流程。
        parent_document_retrieval: bool = False,  # 是否启用父文档检索
        llm_reranking: bool = False,              # 是否启用LLM重排
        llm_reranking_sample_size: int = 5,
        top_n_retrieval: int = 10,
        parallel_requests: int = 10, # 支持并行处理，提升吞吐量
        api_provider: str = "dashscope", # openai
        answering_model: str = "qwen-turbo-latest", # gpt-4o-2024-08-06
        full_context: bool = False
    ):
        # 初始化问题处理器，配置检索、模型、并发等参数
        self.questions = self._load_questions(questions_file_path) # 需要解析json，所以调用了函数
    ...
    def _extract_references(self, pages_list: list, company_name: str) -> list:
        # 根据公司名和页码列表，提取引用信息
        # 由于不再使用subset.csv，返回简化的引用格式
        refs = []
        for page in pages_list:
            refs.append({"pdf_sha1": "", "page_index": page})
        return refs
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        t1 = time.time() # 记录初始化检索结束时间
        print(f"[计时] [get_answer_for_company] 检索器初始化耗时: {t1-t0:.2f} 秒")
        if self.full_context: # 默认是false
    ...
    def _extract_companies_from_subset(self, question_text: str) -> list[str]:
        """从问题文本中提取公司名，现在直接返回空列表，因为不再使用subset.csv"""
        # 由于不再使用subset.csv，返回空列表
        # 实际应用中可能需要根据具体需求实现其他公司提取逻辑
        return []

    def process_question(self, question: str, schema: str):
        # 处理单个问题，不再依赖公司名称提取
        # 直接调用get_answer_for_company，使用空字符串作为公司名
        answer_dict = self.get_answer_for_company(company_name="", question=question, schema=schema)
        return answer_dict
